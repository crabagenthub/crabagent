...
| `iseeagentc` | Go 观测 API（Web 默认对接），默认监听 **8087** |
| `services/collector` | Node Collector（可选），默认 **8787**；`pnpm dev:collector` |
...
pnpm dev             # 并行：Node Collector(8787) + Web；Web 默认直连 iseeagentc `http://127.0.0.1:8087`（见 `loadCollectorUrl`）。仅联调 Node 时在设置里把 Collector URL 指到 Node 根地址（如 `http://127.0.0.1:8787`）
pnpm dev:collector   # 仅 Node Collector，默认 http://127.0.0.1:8787（与 iseeagentc 8087 可同时起）
pnpm dev:web         # 仅 Web；默认请求直连 Go 8087，访问 /zh-CN 或 /en
pnpm build
