...
1. **链路上报**符合行业通用模型（Trace / Span），便于对接 Grafana Tempo、云 APM 等可观测后端。
...
- **Span 存储**：规划为 **独立逻辑模型**（可落在 SQLite 新表、或外部 Tempo 等，或二者兼有）；**标准 Trace 的权威展示**以 **Span 模型 + trace_id** 为准。
...
| 开发者 / 运维 | 同一链路在外部 trace 后端与 Crabagent UI 中 **trace_id** 一致。 |
